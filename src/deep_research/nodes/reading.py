...
async def summarize_text(text: str, topic: str, research_plan: dict) -> str:
    """对给定文本进行总结"""
...
async def read_and_summarize_page(url: str, topic: str, research_plan: dict) -> str:
    ...
    try:
        ...
        return await summarize_text(original_text, topic, research_plan)
...
async def read_and_summarize(state: MainAgentState) -> dict:
    ...
    for url in reading_list:
        if url in raw_content_map:
            tasks.append(summarize_text(raw_content_map[url], topic, research_plan))
        else:
